...
from src.store import add_invocation, list_audit
...
def invoke_tool(
    ...
    if not decision.allowed:
        audit(tool_call_id, caller_brand, name, payload, "brand_denied", started_at, decision.denial_reason)
        raise HTTPException(status_code=403, detail=decision.denial_reason)
    result, summary = spec.execute(args)
    audit(tool_call_id, caller_brand, name, payload, "success", started_at, None, summary)
    return ToolInvokeResponse(tool_call_id=tool_call_id, result=result)


@router.get("/audit")
def get_audit() -> list[AuditRecord]:
    """Return all invocation audit records."""
    return list_audit()
...
def validate_args(
    ...
    try:
        ...
        audit(tool_call_id, caller_brand, tool_name, payload, "schema_invalid", started_at, str(exc))
